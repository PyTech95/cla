...
export default function Nav() {
    ...
    return (
        <header
            ...
            className={`fixed top-0 inset-x-0 z-40 transition-all duration-500 ${scrolled ? "bg-noir/85 backdrop-blur-xl border-b border-white/10" : "bg-transparent"}`}
        >
            <div className="wrap flex items-center justify-between py-3">
                <a href="#home" data-testid="nav-logo" className="flex items-center gap-3 shrink-0">
                ...
                <nav className="hidden lg:flex items-center gap-8">
                    {links.map((l) => (
                        <a key={l.href} href={l.href} data-testid={`nav-link-${l.label.toLowerCase()}`} className="link-sweep text-[12px] tracking-[0.22em] uppercase text-bone/75 hover:text-gold-light transition-colors">
                            {l.label}
                        </a>
                    ))}
                </nav>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <a href="#contact" data-testid="nav-inquire-btn" className="btn-gold hidden sm:inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-[12px] tracking-[0.2em] uppercase">
                        ...
                        <SheetContent side="right" className="bg-noir-2 border-white/10 text-bone w-[86vw] max-w-sm p-0">
                            <div className="flex flex-col h-full">
                                ...
                                <nav className="flex-1 px-3 py-4 overflow-y-auto">
                                    {links.map((l, i) => (
                                        <a key={l.href} href={l.href} data-testid={`mnav-link-${l.label.toLowerCase()}`} onClick={() => setOpen(false)} style={{ animationDelay: `${i * 55}ms` }} className="menu-item flex items-center justify-between px-4 py-4 rounded-2xl text-lg font-serif hover:bg-white/5 active:scale-[.98] transition-transform">
                                            {l.label} <ChevronRight className="w-4 h-4 text-gold" />
                                        </a>
                                    ))}
                                </nav>
                                <div className="px-6 py-6 border-t border-white/10 space-y-3 pb-safe">
                                    <a href="#contact" onClick={() => setOpen(false)} className="btn-gold w-full inline-flex items-center justify-center gap-2 rounded-full py-3.5 text-xs uppercase tracking-[0.24em]">
